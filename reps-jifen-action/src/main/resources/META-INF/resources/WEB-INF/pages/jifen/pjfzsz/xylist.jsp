			...
			<reps:gridrow>
				...
				<reps:gridfield title="操作" width="18">
					...
					<c:if test="${data.isEnable=='0'}">
						<reps:ajax  cssClass="start-use-table" url="isenable.mvc?id=${data.id}&isEnable=1" confirm="你确定要启用吗？" redirect="xylist.mvc" value="启用"></reps:ajax>
					</c:if>
					<c:if test="${data.isEnable=='1'}">
						<reps:ajax  cssClass="stop-use-table" url="isenable.mvc?id=${data.id}&isEnable=0" confirm="你确定要禁用吗？" redirect="xylist.mvc" value="禁用"></reps:ajax>
					</c:if>
				</reps:gridfield>
			</reps:gridrow>
		</reps:grid>
