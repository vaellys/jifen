...
@RestController
@RequestMapping(value = "/oapi/pointscollect")
public class PointsCollectORest extends RestBaseController{
	
	private static Log logger = LogFactory.getLog(PointsCollectORest.class);
	
	@Autowired
	private IPointsCollectService jfPointsCollectService;
	
	@RequestMapping(value = "/list", method = { RequestMethod.GET })
	public RestResponse<ListResult<PointsCollect>> list(PointsCollect pointsCollect, Integer pageIndex, Integer pageSize) {
		try {
			ListResult<PointsCollect> result = jfPointsCollectService.query(pointsCollect, pageIndex, pageSize);
